...
async function fetchData(questId) {
   ...
   try {
      ...
      logger.error("There was an error fetching the quest data", error);
      return null;
   }
}
      ...
      try {
         ...
         if (fromBlock < toBlock) {
            ...
            for (let i = 0; i < events.length; i++) {
               ...
               logger.info(`New Quest Created: ${questId}`);
               ...
               setTimeout(async () => {
                  ...
                  if (questData) {
                     ...
                     logger.info(`Sending discord notification for ${questId}`);
                     await sendMessage(questData);

                     // send notification to twitter (delayed for metadata image to cache)
                     setTimeout(async () => {
                        logger.info(`Sending twitter notification for ${questId}`);
                        await sendTweet(questData);
                     }, 60_000);
                  }
