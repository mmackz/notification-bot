...
async function sendNotification(questData) {
   const { description, icon, name, questId, rewardData } = questData;
   const decimals = rewardData[0].tokenSymbol === "USDC" ? 10 ** 6 : 10 ** 18;
   const rewards = {
      token: rewardData[0].tokenSymbol,
      amount: rewardData[0].amount / decimals,
   ...
   try {
      ...
      if (role.position > questbot.roles.botRole.position) {
         return interaction.reply({
            ...
            ephemeral: true
         }).catch((e) => console.log("An error occured while replying to the interaction"));
      }
