...
async function sendTweet(url, data) {
...
async function shortenUrl(url) {
   ...
   for (const shortener of urlShorteners) {
      try {
         ...
         console.error(`Failed to shorten URL with ${shortener.name}:`, error);
         continue;
      }
   }

   // If no shortener is available, return original url
   console.error(`Failed to shorten URL, returning original url: ${url}`);
   return url;
}
