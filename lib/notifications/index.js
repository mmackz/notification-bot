...
async function createChannels(guild, role) {
   ...
   if (!setupChannel) {
      ...
      logger.info(`🔔-notification-setup channel has been created in ${guild.name}`);
   }
   ...
   if (!notificationsChannel) {
      ...
      logger.info(`🔔-notifications channel has been created in ${guild.name}`);
   }
}

async function createEmbed(data) {
   const { questId, name, description, rewards, questCreator } = data;
   let icon = data.icon;
   ...
   return new EmbedBuilder()
      ...
      .setURL(
         `https://rabbithole.gg/quests/${questId}?wallet_ref=${process.env.REF_ADDRESS}`
      )
      .setAuthor({
         name: "Rabbithole",
      ...
      .addFields(
         ...
         { name: "Creator", value: questCreator, inline: false },
...
async function createRole(guild) {
   ...
   if (!role) {
      logger.info(`Quester role created in ${guild.name}`);
...
async function getRewardEmoji(data) {
   ...
   if (!emoji) {
      ...
      logger.info(`Emoji for ${data.token} was created.`);
   }
   return emoji;
}

async function sendNotification(questData) {
   const { description, icon, name, questId, rewardData, usdValue, questCreator } =
      questData;
   const rewards = {
   ...
   try {
      ...
      logger.error(`An error while trying to post a new notification\n`, error);
   }
}
...
async function setupGuild(guild) {
   try {
      ...
      logger.error(`An error occurred while setting up the bot in ${guild.name}\n`, error);
   }
}
...
async function setupRoleMessage(channel) {
   ...
   try {
      ...
      logger.error("There was an error creating the role setup message:\n", error);
   }
}
   ...
   try {
      ...
      if (role.position > questbot.roles.botRole.position) {
         return interaction
            ...
            .catch((e) =>
               logger.info("An error occured while replying to the interaction")
            );
      }
      ...
      logger.error("An error occurred in the toggleRole function:\n", error);
      interaction.reply({
         content:
...
async function notifyMirrorPost({ digest, title }) {
   logger.info(`New mirror post: ${title}`);
